...
function index()
	redir = luci.http.formvalue("redir", true) or
	  luci.dispatcher.build_url(unpack(luci.dispatcher.context.request))

	--get other element
	--tabRedir.redir1   = luci.http.formvalue("redir1", true)
	--tabRedir.redir2  = luci.http.formvalue("redir2", true)
...
function ApConfSave()
	...
	luci.template.render("admin_uci/ApConfSave", {
		changes = next(changes) and changes,
		configs = reload
	})
end
